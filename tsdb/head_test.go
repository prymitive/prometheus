...
func TestMemSeries_truncateChunks(t *testing.T) {
	...
	for i := 0; i < 4000; i += 5 {
		ok, _, _ := s.append(int64(i), float64(i), 0, chunkDiskMapper)
...
func TestMemSeries_append(t *testing.T) {
	...
	for i := 1; i < 1000; i++ {
		ok, _, _ := s.append(1001+int64(i), float64(i), 0, chunkDiskMapper)
		require.True(t, ok, "append failed")
	}
...
func TestGCChunkAccess(t *testing.T) {
	...
	ok, chunkCreated, _ := s.append(0, 0, 0, h.chunkDiskMapper)
	require.True(t, ok, "series append failed")
	require.True(t, chunkCreated, "chunks was not created")
	ok, chunkCreated, _ = s.append(999, 999, 0, h.chunkDiskMapper)
	require.True(t, ok, "series append failed")
	require.False(t, chunkCreated, "chunks was created")

	// A new chunks should be created here as it's beyond the chunk range.
	ok, chunkCreated, _ = s.append(1000, 1000, 0, h.chunkDiskMapper)
	require.True(t, ok, "series append failed")
	require.True(t, chunkCreated, "chunks was not created")
	ok, chunkCreated, _ = s.append(1999, 1999, 0, h.chunkDiskMapper)
	require.True(t, ok, "series append failed")
	require.False(t, chunkCreated, "chunks was created")
...
func TestHeadReadWriterRepair(t *testing.T) {
	...
	// Fill the chunk segments and corrupt it.
	{
		...
		for i := 0; i < 7; i++ {
			ok, chunkCreated, _ := s.append(int64(i*chunkRange), float64(i*chunkRange), 0, h.chunkDiskMapper)
			require.True(t, ok, "series append failed")
			require.True(t, chunkCreated, "chunk was not created")
			ok, chunkCreated, _ = s.append(int64(i*chunkRange)+chunkRange-1, float64(i*chunkRange), 0, h.chunkDiskMapper)
			require.True(t, ok, "series append failed")
...
func TestIsolationAppendIDZeroIsNoop(t *testing.T) {
	...
	ok, _, _ := s.append(0, 0, 0, h.chunkDiskMapper)
	require.True(t, ok, "Series append failed.")
