...
func (sl *scrapeLoop) scrapeAndReport(last, appendTime time.Time, errc chan<- error) time.Time {
	...
	var total, added, seriesAdded, overLimit, bytesRead int
	...
	defer func() {
		if err = sl.report(app, appendTime, time.Since(start), total, added, seriesAdded, overLimit, bytesRead, scrapeErr); err != nil {
...
loop:
	...
	if appErrs.numOverLimit > 0 {
		level.Debug(sl.l).Log("msg", "HEAD series limit reached, cannot append new series", "num_dropped", appErrs.numOverLimit)
		if err == nil {
	...
	if err == nil {
		sl.cache.forEachStale(func(lset labels.Labels) bool {
			...
			switch {
			case errors.Is(err, storage.ErrOutOfOrderSample), errors.Is(err, storage.ErrDuplicateSampleForTimestamp), errors.Is(err, storage.ErrCapacityExhaused):
...
var (
	...
	scrapeSeriesOverLimitMetricName = []byte("scrape_series_over_limit" + "\xff")
...
func (sl *scrapeLoop) report(app storage.Appender, start time.Time, duration time.Duration, scraped, added, seriesAdded, overLimit, bytes int, scrapeErr error) (err error) {
	...
	if err = sl.addReportSample(app, scrapeSeriesOverLimitMetricName, ts, float64(overLimit), b); err != nil {
		return
	}
...
func (sl *scrapeLoop) reportStale(app storage.Appender, start time.Time) (err error) {
	...
	if err = sl.addReportSample(app, scrapeSeriesOverLimitMetricName, ts, stale, b); err != nil {
		return
	}
	if sl.reportExtraMetrics {
